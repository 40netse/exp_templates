...
variable "igw_id" {
  description = "Internet Gateway id for the security VPC."
  type        = string
  default     = ""
}
...
variable "security_vpc_tgw_attachments" {
  description = <<-EOF
    Transit gateway VPC attachments configuration for the Transit gateway. 
    Format:
    ```
        security_vpc_tgw_attachments = {
            \<Option\> = \<Option value\>
        }
    ```
    Options:
        - appliance_mode_support                          : (Optional|string) Whether Appliance Mode support is enabled. If enabled, a traffic flow between a source and destination uses the same Availability Zone for the VPC attachment for the lifetime of that flow. Valid values: disable, enable. Default value: enable.
...
variable "spk_vpc" {
  description = <<-EOF
    Spoke VPC configuration. VPC ID and Subnet IDs should be provided. This module will create Transit Gateway Attachments under each subnet, and create VPC route tables with the subnet associated.
    Format:
    ```
        spk_vpc = {
            \<Spoke VPC name\> = {
...
variable "enable_east_west_inspection" {
  description = "If true, east-west security inspection will be enabled."
  type        = bool
  default     = true
}
